...
namespace Files.App.ViewModels
{
	...
	public sealed partial class ShellViewModel : ObservableObject, IDisposable
	{
		...
		// [OPTIMIZATION] Biến này dùng làm "Van điều tiết" (Throttling) để chống đứng hình UI
		private DateTime lastIntermediateUpdateTime = DateTime.MinValue;
		...
		private async Task RapidAddItemsToCollectionAsync(string path, string? previousDir, Action postLoadCallback)
		{
			...
			try
			{
				...
				// [OPTIMIZATION] Khởi tạo van điều tiết thời gian khi bắt đầu đọc thư mục
				...
				else
				{
					await Task.Run(async () =>
					{
						List<ListedItem> fileList = await Win32StorageEnumerator.ListEntries(path, hFile, findData, cancellationToken, -1, intermediateAction: async (intermediateList) =>
						{
							...
							// [OPTIMIZATION] Van điều tiết giao diện (UI Throttling)
							// Tránh "Collection Changed Storm" - Khựng hình vì UI vẽ lại hàng chục lần một giây
			...
			await Task.Run(async () =>
			{
				List<ListedItem> finalList = await UniversalStorageEnumerator.ListEntries(
					...
					async (intermediateList) =>
					{
						...
						// [OPTIMIZATION] Khắc phục lỗi vòng lặp Sắp Xếp chết người (O(N^2) Lag)
						// Lệnh OrderFilesAndFoldersAsync() CŨ nằm ở đây bắt UI sắp xếp hàng ngàn file liên tục.
						// Bỏ lệnh Sort đi, và thêm Van Điều Tiết UI 500ms.
						if ((DateTime.UtcNow - lastIntermediateUpdateTime).TotalMilliseconds > 500)
						{
							lastIntermediateUpdateTime = DateTime.UtcNow;
							await ApplyFilesAndFoldersChangesAsync();
				...
				// Chỉ sắp xếp MỘT LẦN DUY NHẤT ở cuối cùng khi đã tải xong toàn bộ File
				await OrderFilesAndFoldersAsync();
				await ApplyFilesAndFoldersChangesAsync();
			}, cancellationToken);
