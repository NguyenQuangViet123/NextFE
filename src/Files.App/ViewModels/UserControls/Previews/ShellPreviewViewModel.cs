...
namespace Files.App.ViewModels.Previews
{
	public sealed partial class ShellPreviewViewModel : BasePreviewModel
	{
		...
		// [OPTIMIZATION] Lưu trữ tên Class để tránh lỗi GDI Memory Leak
		// Con trỏ lpszClassName của C# có thể bị GC dọn mất, phải dùng biến string neo lại.
		string? _className;
		...
		public static unsafe Guid? FindPreviewHandlerFor(string extension, nint hwnd)
		{
			...
			try
			{
				// [OPTIMIZATION] Sửa lỗi Buffer Overflow và Zero-Allocation.
				// Dùng stackalloc tống mảng vào thanh ghi CPU thay vì new char[] vào RAM. Tốc độ ánh sáng.
				char* pszOutput = stackalloc char[1024];
					...
					pszExtra = "{8895b1c6-b41f-4c1c-a562-0d564250836f}")
				{
					...
					// [FIX] Đồng bộ size cchOutput với pszOutput để C++ không ghi đè RAM gây sập app.
					uint cchOutput = 1024;
		...
		private unsafe LRESULT WndProc(HWND hwnd, uint msg, WPARAM wParam, LPARAM lParam)
		{
			if (msg is PInvoke.WM_CREATE)
			{
				...
				try
				{
					// [OPTIMIZATION] Check Cancel để tránh khởi tạo COM nặng nề nếu user đã lướt đi
					if (!LoadCancelledTokenSource.Token.IsCancellationRequested && clsid.HasValue)
					{
		...
		{
			// [OPTIMIZATION] Chốt chặn: Không tốn công tạo HWND nếu Preview bị Cancel
			...
			// Lưu thẳng vào biến class để dùng lại lúc Unload
			_className = $"{nameof(ShellPreviewViewModel)}-{Guid.NewGuid()}";
			var szWindowName = $"Preview";

			fixed (char* pszClassName = _className)
			{
		...
		{
			if (LoadCancelledTokenSource.Token.IsCancellationRequested) return false;
		...
		public unsafe void UnloadPreview()
		{
			if (_hWnd != HWND.Null)
			{
				...
				// [SAFETY] Trả HWND về Null để tránh hủy hai lần
				_hWnd = HWND.Null;
			}
			else
			{
			...
			// [OPTIMIZATION] Hủy đăng ký Window Class bằng chuỗi gốc _className.
			// Khắc phục lỗi rò rỉ GDI Class Atom làm sập UI hệ thống.
			if (_className != null)
			{
				fixed (char* pszClassName = _className)
				{
					PInvoke.UnregisterClass(pszClassName, PInvoke.GetModuleHandle(default(PWSTR)));
				}
				_className = null;
			}
		}
