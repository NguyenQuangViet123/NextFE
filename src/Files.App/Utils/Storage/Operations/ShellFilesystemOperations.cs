...
namespace Files.App.Utils.Storage
{
	...
	public sealed partial class ShellFilesystemOperations : IFilesystemOperations
	{
		...
		{
			// [OPTIMIZATION] Lắp Van điều tiết UI: Chỉ cho phép gửi tín hiệu vẽ UI 500ms/lần.
			// Tránh "Cơn bão UI" (UI Storm) gây giật lag hoặc Crash khi copy 100,000 files.
			var throttledProgress = new ThrottledProgress<StatusCenterItemProgressModel>(progress, 500);
				...
				|| destination.Any(x => string.IsNullOrWhiteSpace(x) || x.StartsWith(@"\\?\", StringComparison.Ordinal) || FtpHelpers.IsFtpPath(x) || ZipStorageFolder.IsZipPath(x, false)))
			{
				...
				return await _filesystemOperations.CopyItemsAsync(source, destination, collisions, throttledProgress, cancellationToken);
			}

			// Giữ lại progress gốc cho fsProgress để khi báo Success (100%), nó lọt qua được Van ngay lập tức
			...
			if (sourceRename.Any())
			{
				var resultItem = await FileOperationsHelpers.CopyItemAsync(sourceRename.Select(s => s.Path).ToArray(), destinationRename.ToArray(), false, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
			...
			{
				var resultItem = await FileOperationsHelpers.CopyItemAsync(sourceReplace.Select(s => s.Path).ToArray(), destinationReplace.ToArray(), true, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
		...
		public async Task<(IStorageHistory, IStorageItem)> CreateAsync(IStorageItemWithPath source, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken, bool asAdmin = false)
		{
			...
			{
				// Fallback to built-in file operations
				return await _filesystemOperations.CreateAsync(source, throttledProgress, cancellationToken);
			}
		...
		public async Task<IStorageHistory> CreateShortcutItemsAsync(IList<IStorageItemWithPath> source, IList<string> destination, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken)
		{
			...
			// [OPTIMIZATION] Van điều tiết cho vòng lặp ngầm
			...
			// Báo cáo cái cuối cùng để thanh hiển thị 100%
			fsProgress.Report();
		...
		public async Task<IStorageHistory> DeleteItemsAsync(IList<IStorageItemWithPath> source, IProgress<StatusCenterItemProgressModel> progress, bool permanently, CancellationToken cancellationToken, bool asAdmin = false)
		{
			...
			{
				// Fallback to built-in file operations
				return await _filesystemOperations.DeleteItemsAsync(source, throttledProgress, permanently, cancellationToken);
			}
			...
			var (success, response) = await FileOperationsHelpers.DeleteItemAsync(deleteFilePaths.ToArray(), permanently, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
			...
			else
			{
				...
				else if (deleteResult.Items.All(x => x.HResult == -1) && permanently) // ADS
				{
					...
					return await _filesystemOperations.DeleteItemsAsync(sourceMatch, throttledProgress, permanently, cancellationToken);
				}
		...
		public async Task<IStorageHistory> MoveItemsAsync(IList<IStorageItemWithPath> source, IList<string> destination, IList<FileNameConflictResolveOptionType> collisions, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken, bool asAdmin = false)
		{
			...
			{
				// Fallback to built-in file operations
				return await _filesystemOperations.MoveItemsAsync(source, destination, collisions, throttledProgress, cancellationToken);
			}
			...
			if (sourceRename.Any())
			{
				var (status, response) = await FileOperationsHelpers.MoveItemAsync(sourceRename.Select(s => s.Path).ToArray(), destinationRename.ToArray(), false, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
			...
			if (sourceReplace.Any())
			{
				var (status, response) = await FileOperationsHelpers.MoveItemAsync(sourceReplace.Select(s => s.Path).ToArray(), destinationReplace.ToArray(), true, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
		...
		public async Task<IStorageHistory> RestoreItemsFromTrashAsync(IList<IStorageItemWithPath> source, IList<string> destination, IProgress<StatusCenterItemProgressModel> progress, CancellationToken cancellationToken, bool asAdmin = false)
		{
			...
			{
				// Fallback to built-in file operations
				return await _filesystemOperations.RestoreItemsFromTrashAsync(source, destination, throttledProgress, cancellationToken);
			}
			...
			var moveResult = new ShellOperationResult();
			var (status, response) = await FileOperationsHelpers.MoveItemAsync(source.Select(s => s.Path).ToArray(), [.. destination], false, MainWindow.Instance.WindowHandle.ToInt64(), asAdmin, throttledProgress, operationID);
		...
		// [OPTIMIZATION] Lớp trung gian chặn các báo cáo UI thừa thãi để bảo vệ tài nguyên CPU và RAM
		private sealed class ThrottledProgress<T> : IProgress<T>
		{
			private readonly IProgress<T> _baseProgress;
			private readonly double _intervalMs;
			private DateTime _lastUpdate = DateTime.MinValue;

			public ThrottledProgress(IProgress<T> baseProgress, double intervalMs = 500)
			{
