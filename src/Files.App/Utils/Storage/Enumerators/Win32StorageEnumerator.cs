			...
			do
			{
					...
					(!startWithDot || userSettingsService.FoldersSettingsService.ShowDotFiles))
				{
					if (((FileAttributes)findData.dwFileAttributes & FileAttributes.Directory) != FileAttributes.Directory)
					{
						...
						if (file is not null)
						{
							// [VIP LAZY EVALUATION] Áp dụng triết lý phản biện:
							// Loại bỏ hoàn toàn việc nạp Icon (PreloadedIconData) tại đây để tránh Task Explosion (Bùng nổ RAM/CPU).
							// Hệ thống sẽ trả về Icon mặc định siêu nhẹ. Việc nạp Icon thật sẽ được WinUI tự động kích hoạt
							// thông qua hàm LoadExtendedItemPropertiesAsync KHI VÀ CHỈ KHI file này hiển thị trên màn hình.
					...
					else if (((FileAttributes)findData.dwFileAttributes & FileAttributes.Directory) == FileAttributes.Directory)
					{
						if (findData.cFileName != "." && findData.cFileName != "..")
						{
							...
							{
								// [VIP LAZY EVALUATION] Tương tự như file, xóa bỏ nạp Icon trước cho Folder.
