...
namespace Files.App.Utils.FileTags
{
	...
	public static class FileTagsHelper
	{
		...
		{
			// [OPTIMIZATION] Khắc phục triệt để lỗi giật lag, đơ UI khi hệ thống có hàng ngàn file được gắn tags.
			// Hàm UpdateTagsDb() cũ chạy đồng bộ (Synchronous) và thực hiện đọc/ghi ổ đĩa liên tục (I/O Blocking).
			// Bọc toàn bộ logic này vào Task.Run để chuyển nó sang Background Thread, giúp UI mượt mà ngay lập tức.
			Task.Run(() =>
			{
				var dbInstance = GetDbInstance();

				// [SAFETY] Thêm .ToList() để copy danh sách ra bộ nhớ đệm (Snapshot).
				// Tránh crash ứng dụng (Collection modified exception) khi có tác vụ ngầm khác can thiệp vào DB cùng lúc.
				var allFiles = dbInstance.GetAll().ToList();
